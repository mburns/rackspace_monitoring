...
action :create do
  ...
  remote_file "/usr/lib/rackspace-monitoring-agent/plugins/#{file}" do
    source source
    owner 'root'
    group 'root'
    mode '00755'
  ...
  template "#{confd}/monitoring-plugin-#{name}.yaml" do
    cookbook cookbook
...
action :delete do
  file "/usr/lib/rackspace-monitoring-agent/plugins/#{file}" do
    action :delete
  end

  file "#{confd}/monitoring-plugin-#{plugin_name}.yaml" do
    action :delete
  end
end
